...
namespace SAPO.utilities
{
    internal class DeviceManipulator
    {
        ...
        public ICollection<UserInterface> GetAllUserInfo(ZkemClient objZkeeper, int machineNumber, ILogger logger)
        {
            ...
            while (objZkeeper.SSR_GetAllUserInfo(machineNumber, out sdwEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))
            {
                for (idwFingerIndex = 0; idwFingerIndex < 10; idwFingerIndex++)
                {
                    ...
                    {
                        UserInterface fpInterface = new UserInterface();
                        fpInterface.MachineNumber = machineNumber;
                        fpInterface.EnrollNumber = int.Parse(sdwEnrollNumber);
                        fpInterface.Name = sName;
                        fpInterface.FingerIndex = idwFingerIndex;
                        fpInterface.TmpData = sTmpData;
                        fpInterface.Privelage = iPrivilege;
                        fpInterface.Password = sPassword;
                        fpInterface.Enabled = bEnabled;
                        fpInterface.iFlag = iFlag.ToString();

                        lstFPTemplates.Add(fpInterface);
                    }
                }
        ...
        public ICollection<MachineInterface> GetLogData(ZkemClient objZkeeper, int machineNumber, ILogger logger)
        {
            ...
            ICollection<MachineInterface> lstEnrollData = new List<MachineInterface>();
...

            {
                ...
                MachineInterface objInterface = new MachineInterface();
                objInterface.MachineNumber = machineNumber;
                objInterface.IndRegID = int.Parse(dwEnrollNumber1);
                objInterface.DateTimeRecord = inputDate;
                objInterface.InOutMode = dwInOutMode;
                objInterface.VerifyMode = dwVerifyMode;

                lstEnrollData.Add(objInterface);
            }

            return lstEnrollData;
        }

        public ICollection<UserIDInterface> GetAllUserID(ZkemClient objZkeeper, int machineNumber)
        {
            ...
            ICollection<UserIDInterface> lstUserIDInfo = new List<UserIDInterface>();
            ...
            {
                UserIDInterface userID = new UserIDInterface();
        ...
        public bool UploadFTPTemplate(ZkemClient objZkeeper, int machineNumber, List<UserInterface> lstUserInfo)
        {
            ...
            if (objZkeeper.BeginBatchUpdate(machineNumber, iUpdateFlag))
            {
                ...
                {
                    sdwEnrollNumber = lstUserInfo[i].EnrollNumber.ToString();
